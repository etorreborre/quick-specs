...
abstract class CongruenceClass extends EqualityFlattener {
  ...
  def add(equality: Equality[Curried]) {
	if (equality.isTautology) {
  	  println("    not adding the tautology"+equality)
	} else {
      println("    adding "+equality)
      initialize(equality)
	  recomputeCongruence()
  ...
  def isCongruent(equality: Equality[Curried]): Boolean = {
    ...
    val congruent = flattened.forall { 
	  case e @ Equality(c @ Curry(_), Apply(a, b)) => 
	    println("lookup for "+e+" "+(representative.get(c), lookup.get((a, b)).map(representative(_))))
	  ...
	  case e @ Equality(Apply(a, b), c @ Curry(_)) => 
	    ...
	    lookup.get((a, b)).map(representative(_)) == representative.get(c)
	  case e @ Equality(a, b) => 
	    println("rep for "+e+" "+(representative.get(a), representative.get(b)))
	    representative.get(a).isDefined && 
	    representative.get(a) == representative.get(b)
	  }
	  println("    "+equality+" is congruent "+congruent)
	  congruent
  }
  
  private def initialize(equality: Equality[Curried]) {
	println("initializing with " + flattenCurried(equality).mkString("\n"))
  ...
  private def update(ra: Curried, rb: Curried) {
	println("    classList "+ra+" "+classList(ra))
	println("    classList "+rb+" "+classList(rb))
