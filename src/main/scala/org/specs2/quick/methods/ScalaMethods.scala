...
class ScalaClassMethods(val declaringClass: Class[_], private val scalaMethods: List[ScalaMethod]) extends 
  TypesMatcher with ScalaMethods {
  ...
  /** @return the list of accepted methods */
  def methods: List[ScalaMethod] = scalaMethods
  /** 
...
trait ScalaMethodsFactory {
  ...
  implicit def toMethods[T <: AnyRef](a: T)(implicit m: ClassManifest[T]): ScalaClassMethods = ScalaMethods.create(a)
